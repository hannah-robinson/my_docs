...
```html
{% assign count = 0 %}  {%- for block in section.blocks -%}   {% assign count =
count | plus: 1 %}     {%- comment -%} Slide goes here. Inside <img /> tag add:
  {% if count > 1 %} loading="lazy" {% endif %}   {%- endcomment -%} {%- endfor
-%}
```
...
             {{ block.settings.image | image_url }} {{ block.settings.image.width }}w"
    ...
    count
  />
  1 %} loading="lazy" {% endif %}   class="{%- if block.settings.image_mob -%}
  hide--small{%- endif -%}"   alt="{{ block.settings.image.alt | escape }}"
    width="{{ block.settings.image.width }}"   height="{{
  block.settings.image.width | divided_by: block.settings.image.aspect_ratio |
  round }}"            > %- else -%}
</div>
```
